...
state_file = "promotion/promotion_state.txt"
output_file = "~/CRAWLER_LLUMAR/result_promotion_scraping_results.csv"
...
try:
    ...
    promotion_data = existing_data.values.tolist()
except FileNotFoundError:
    promotion_data = []
    ...
    while True:
        ...
        for row in rows:
            ...
            if len(cols) == 4:
                ...
                if page_number > 1 and "NOTICE" in promo_no.upper():
                    print(f"Skipping NOTICE on page {page_number}: {promo_no}")
                    continue
        ...
        promotion_df.to_csv(output_file, index=False, encoding='utf-8-sig')
